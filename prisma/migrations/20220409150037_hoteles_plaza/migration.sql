/*
  ...
  - You are about to drop the `tacos` table. If the table is not empty, all the data it contains will be lost.
...
CREATE TABLE `ActivityOfDeleteData` (
    ...
    UNIQUE INDEX `ActivityOfDeleteData_userId_key`(`userId`),
    PRIMARY KEY (`id`)
...
CREATE TABLE `Departments` (
    ...
    `registredBy` VARCHAR(60) NOT NULL,
...
-- AddForeignKey
ALTER TABLE `ActivityOfDeleteData` ADD CONSTRAINT `ActivityOfDeleteData_userId_fkey` FOREIGN KEY (`userId`) REFERENCES `Users`(`id`) ON DELETE RESTRICT ON UPDATE CASCADE;
