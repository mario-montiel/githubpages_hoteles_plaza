...
CreateUser.getInitialProps = async (ctx: NextPageContext) => {
    ...
    departmentsJson = await getDepartmentAndHotels(mainUrl + '/api/admin/departments/showDepartments', ctx)
    typeUsersJson = await getDepartmentAndHotels(mainUrl + '/api/admin/users/showTypeUsers', ctx)
    hotelsJson = await getDepartmentAndHotels(mainUrl + '/api/admin/hotels/showHotels', ctx)
...
async function getDepartmentAndHotels(url: string, ctx: NextPageContext) {
    ...
    if (resp.status === 401 && !ctx.req) {
        Router.replace(mainUrl + '/aG90ZWxlc19wbGF6YQ0K/authentication/login')
        return {};
    }

    if (resp.status === 401 && ctx.req) {
        ctx.res?.writeHead(302, {
            Location: mainUrl + '/aG90ZWxlc19wbGF6YQ0K/authentication/login'
        })
        ctx.res?.end()
        return;
    }
